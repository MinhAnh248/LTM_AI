...
const Dashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const catData = Array.isArray(categoryRes.data) ? categoryRes.data : [];
      const dailyDataRaw = Array.isArray(dailyRes.data) ? dailyRes.data : [];
      
      console.log('Category data:', catData);
      console.log('Daily data:', dailyDataRaw);
      
      setCategoryData(catData.length > 0 ? catData : [{category: 'Chưa có dữ liệu', amount: 0}]);
      setDailyData(dailyDataRaw.length > 0 ? dailyDataRaw : [{date: new Date().toISOString().split('T')[0], amount: 0}]);
    } catch (error) {
    ...
    <DashboardContainer>
      ...
      <ChartsGrid>
        <ChartCard>
          ...
          {categoryData && categoryData.length > 0 ? (
            ...
            <div style={{textAlign: 'center', padding: '50px', color: '#999'}}>Chưa có dữ liệu chi tiêu</div>
          )}
        </ChartCard>
      ...
      <TimelineChart>
        ...
        {dailyData && dailyData.length > 0 ? (
          ...
          <div style={{textAlign: 'center', padding: '100px', color: '#999'}}>Chưa có dữ liệu chi tiêu</div>
        )}
      </TimelineChart>
    </DashboardContainer>
  );
